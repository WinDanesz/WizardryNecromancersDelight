...
public class EntityStinkBomb extends EntityBomb {

	public EntityStinkBomb(World world){
		super(world);
	}
	...
	protected void onImpact(RayTraceResult rayTrace){
		...
		if(entityHit != null){
			...
			float damage = NDSpells.stink_bomb.getProperty(Spell.DIRECT_DAMAGE).floatValue() * damageMultiplier;
			...
			if(entityHit instanceof EntityLivingBase && !MagicDamage.isEntityImmune(DamageType.POISON, entityHit))
				((EntityLivingBase)entityHit).addPotionEffect(new PotionEffect(ModMobEffects.SOILED,
						NDSpells.stink_bomb.getProperty(Spell.DIRECT_EFFECT_DURATION).intValue(),
						NDSpells.stink_bomb.getProperty(Spell.DIRECT_EFFECT_STRENGTH).intValue()));
		}
		...
		if(!this.world.isRemote){
			...
			double range = NDSpells.stink_bomb.getProperty(Spell.EFFECT_RADIUS).floatValue() * blastMultiplier;
			...
			for(EntityLivingBase target : targets){
				if(target != entityHit && target != this.getThrower()
					...
					target.attackEntityFrom(
							...
							NDSpells.stink_bomb.getProperty(Spell.SPLASH_DAMAGE).floatValue() * damageMultiplier);
					target.addPotionEffect(new PotionEffect(ModMobEffects.SOILED,
							NDSpells.stink_bomb.getProperty(Spell.SPLASH_EFFECT_DURATION).intValue(),
							NDSpells.stink_bomb.getProperty(Spell.SPLASH_EFFECT_STRENGTH).intValue()));
				}
			}
